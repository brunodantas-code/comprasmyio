ALTER TABLE public.approval_settings
  ADD COLUMN IF NOT EXISTS dual_approval_job_title_1_id uuid REFERENCES public.job_titles(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS dual_approval_job_title_2_id uuid REFERENCES public.job_titles(id) ON DELETE SET NULL;

UPDATE public.approval_settings
SET dual_approval_job_title_1_id = COALESCE(
      dual_approval_job_title_1_id,
      (SELECT id FROM public.job_titles WHERE upper(COALESCE(short_name, '')) = 'CFO' AND active ORDER BY created_at LIMIT 1)
    ),
    dual_approval_job_title_2_id = COALESCE(
      dual_approval_job_title_2_id,
      (SELECT id FROM public.job_titles WHERE upper(COALESCE(short_name, '')) = 'CEO' AND active ORDER BY created_at LIMIT 1)
    );

ALTER TABLE public.approval_settings
  ADD CONSTRAINT approval_settings_dual_titles_distinct
  CHECK (
    dual_approval_job_title_1_id IS NULL
    OR dual_approval_job_title_2_id IS NULL
    OR dual_approval_job_title_1_id <> dual_approval_job_title_2_id
  );
