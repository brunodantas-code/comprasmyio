CREATE TABLE public.user_additional_job_titles (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_additional_job_titles TO authenticated;
GRANT ALL ON public.user_additional_job_titles TO service_role;

ALTER TABLE public.user_additional_job_titles ENABLE ROW LEVEL SECURITY;

CREATE POLICY user_additional_job_titles_select_self_or_admin
...
WITH CHECK (
  ...
  AND NOT EXISTS (
    ...
    WHERE p.id = user_id AND p.job_title_id = job_title_id
  )
  AND EXISTS (
    SELECT 1 FROM public.job_titles jt
    WHERE jt.id = job_title_id AND jt.active
  )
);
...
CREATE POLICY user_additional_job_titles_admin_delete
ON public.user_additional_job_titles
FOR DELETE TO authenticated
USING (public.is_access_admin(auth.uid()));

CREATE OR REPLACE FUNCTION public.has_job_title(_user_id uuid, _job_title_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.has_job_title(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_job_title(uuid, uuid) TO service_role;
...
BEGIN
  ...
  SELECT p2.id INTO cfo_id FROM public.profiles p2
  WHERE public.has_job_title_name(p2.id, 'CFO') AND p2.deleted_at IS NULL AND p2.id <> NEW.requester_id
  ORDER BY p2.created_at LIMIT 1;
  SELECT p2.id INTO ceo_id FROM public.profiles p2
  WHERE public.has_job_title_name(p2.id, 'CEO') AND p2.deleted_at IS NULL AND p2.id <> NEW.requester_id
  ORDER BY p2.created_at LIMIT 1;
  ...
  IF NOT auto_ok THEN
    ...
    IF NEW.request_model <> 'dispositivos' AND cfg IS NOT NULL AND cfg.dual_approval_enabled AND v > COALESCE(cfg.dual_approval_threshold, 100000) THEN
      dual_idx := idx + 1;
      FOR u IN
        SELECT DISTINCT ON (title_name) title_name, user_id
        FROM (
          SELECT lower(jt.name) AS title_name, p2.id AS user_id, p2.created_at
          FROM public.profiles p2
          JOIN public.job_titles jt ON jt.id = p2.job_title_id
          WHERE lower(jt.name) IN ('cfo','ceo') AND jt.active AND p2.deleted_at IS NULL
          UNION ALL
          SELECT lower(jt.name) AS title_name, p2.id AS user_id, p2.created_at
          FROM public.profiles p2
          JOIN public.user_additional_job_titles uajt ON uajt.user_id = p2.id
          JOIN public.job_titles jt ON jt.id = uajt.job_title_id
          WHERE lower(jt.name) IN ('cfo','ceo') AND jt.active AND p2.deleted_at IS NULL
        ) eligible
        ORDER BY title_name, created_at
      LOOP
        IF NOT (u.user_id = ANY(seen)) AND (final_id IS NULL OR u.user_id <> final_id) THEN
          INSERT INTO public.approval_steps(order_id, step_index, role_label, approver_id)
...
COMMENT ON TABLE public.operational_functions IS 'DEPRECATED: replaced by job_titles and user_additional_job_titles.';
COMMENT ON TABLE public.user_operational_functions IS 'DEPRECATED: replaced by user_additional_job_titles.';