ALTER TABLE public.approval_settings
  ADD COLUMN ceo_approver_job_title_id uuid REFERENCES public.job_titles(id) ON DELETE SET NULL;

UPDATE public.approval_settings settings
SET ceo_approver_job_title_id = title.id
FROM public.job_titles title
WHERE settings.id
  AND settings.ceo_approver_job_title_id IS NULL
  AND upper(COALESCE(title.short_name, '')) = 'CFO';
