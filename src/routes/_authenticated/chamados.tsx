...
type CallStatus = "aberto" | "em_atendimento" | "aguardando" | "resolvido" | "concluido" | "cancelado";
...
const STATUSES: Record<CallStatus, string> = { aberto: "Aberto", em_atendimento: "Em atendimento", aguardando: "Aguardando", resolvido: "Atendido pelo Suporte", concluido: "Concluído", cancelado: "Cancelado" };
const SUPPORT_STATUSES: Record<Exclude<CallStatus, "concluido">, string> = { aberto: "Aberto", em_atendimento: "Em atendimento", aguardando: "Aguardando", resolvido: "Atendido pelo Suporte", cancelado: "Cancelado" };
...
export const Route = createFileRoute("/_authenticated/chamados")({
  ...
  beforeLoad: async ({ context }) => {
    const [{ data: access }, { data: isSupportAnalyst }] = await Promise.all([
      supabase.from("user_app_access").select("app_key").eq("user_id", context.user.id).eq("app_key", "chamados").maybeSingle(),
      supabase.rpc("is_customer_support_analyst", { _user_id: context.user.id }),
    ]);
    if (!access && !isSupportAnalyst) throw redirect({ to: "/portal" });
  },
  component: CallsPage,
  head: () => ({ meta: [
...
function CallsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: isSupportAnalyst } = await supabase.rpc("is_customer_support_analyst", { _user_id: auth.user.id });
      return { calls: (calls ?? []) as InternalCall[], profiles: (profiles ?? []) as Profile[], userId: auth.user.id, isAdmin: Boolean(admin), canManage: Boolean(admin) || Boolean(isSupportAnalyst), responsible: (profiles ?? []).filter((profile) => accessIds.has(profile.id) && adminIds.has(profile.id)) as Profile[] };
    },
  });
  ...
  const refresh = () => { void qc.invalidateQueries({ queryKey: ["internal-calls"] }); void qc.invalidateQueries({ queryKey: ["pending-actions"] }); };
  return <div className="min-h-screen bg-background">
    <AppHeader logo={<MyioAppLogo appName="OpDesk" className="text-base sm:text-2xl" />} />
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex items-end justify-between gap-4"><div><Phone className="h-8 w-8 text-foreground" aria-hidden="true" /><p className="mt-2 text-muted-foreground">Problemas em campo, reclamações e assuntos internos.</p></div><NewCallDialog open={newOpen} onOpenChange={setNewOpen} userId={data?.userId} onCreated={refresh} /></div>
...
function CallDetails({ call, data, names, onClose, onUpdated }: { call: InternalCall | null; data?: { userId: string; isAdmin: boolean; canManage: boolean; profiles: Profile[]; responsible: Profile[] }; names: Map<string, string>; onClose: () => void; onUpdated: () => void }) {
  ...
  const conclude = useMutation({ mutationFn: async () => { if (!call || call.reporter_id !== data?.userId || call.status !== "resolvido") throw new Error("Este chamado não está disponível para conclusão"); const { error } = await supabase.from("internal_calls").update({ status: "concluido" }).eq("id", call.id).eq("reporter_id", data.userId).eq("status", "resolvido"); if (error) throw error; }, onSuccess: () => { toast.success("Chamado concluído"); onClose(); onUpdated(); }, onError: (error: Error) => toast.error(error.message) });
  ...
  return <Dialog open={Boolean(call)} onOpenChange={(value) => { if (!value) onClose(); }}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">{call ? <div className="space-y-5"><DialogHeader><DialogTitle>#{call.call_number} — {call.title}</DialogTitle><DialogDescription>{CATEGORIES[call.category]} · Solicitante: {names.get(call.reporter_id)} · {new Date(call.created_at).toLocaleString("pt-BR")}</DialogDescription></DialogHeader><p className="whitespace-pre-wrap text-sm">{call.description}</p><div className="flex flex-wrap gap-2"><Badge variant="status">{STATUSES[call.status]}</Badge><Badge variant="outline">Prioridade {PRIORITIES[call.priority]}</Badge>{call.source === "site_survey" ? <Badge variant="outline">Site Survey</Badge> : null}</div>
    ...
    <div className="space-y-3 border-t pt-4"><p className="flex items-center gap-2 font-semibold"><MessageCircle className="h-4 w-4" />Mensagens</p>{details.data?.messages.map((entry) => <div key={entry.id} className="rounded-md bg-muted p-3 text-sm"><div className="mb-1 flex justify-between gap-2 font-semibold"><span>{names.get(entry.author_id)}</span><span className="text-xs font-normal text-muted-foreground">{new Date(entry.created_at).toLocaleString("pt-BR")}</span></div><p className="whitespace-pre-wrap">{entry.message}</p></div>)}{call.status !== "concluido" && call.status !== "cancelado" ? <div><Textarea value={message} onChange={(event) => setMessage(event.target.value)} maxLength={4000} placeholder="Escreva uma mensagem" /><div className="mt-2 flex justify-end"><Button size="sm" onClick={() => send.mutate()} disabled={!message.trim() || send.isPending}><Send className="h-4 w-4" />Enviar</Button></div></div> : null}</div>
    {data?.canManage && call.status !== "concluido" ? <div className="space-y-3 border-t pt-4"><h3 className="font-semibold">Gestão do chamado</h3><div className="grid gap-3 sm:grid-cols-2"><div><Label>Situação</Label><Select value={status} onValueChange={(value) => setStatus(value as CallStatus)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(SUPPORT_STATUSES).map(([key, text]) => <SelectItem key={key} value={key}>{text}</SelectItem>)}</SelectContent></Select></div><div><Label>Responsável</Label><Select value={assignee} onValueChange={setAssignee}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Sem responsável</SelectItem>{data.responsible.map((profile) => <SelectItem key={profile.id} value={profile.id}>{profile.full_name || profile.email}</SelectItem>)}</SelectContent></Select></div></div><div><Label>Observações internas</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} maxLength={4000} /></div><Button onClick={() => update.mutate()} disabled={update.isPending}>Salvar andamento</Button></div> : call.internal_notes ? <p className="whitespace-pre-wrap text-sm">{call.internal_notes}</p> : null}
    {call.status === "resolvido" && call.reporter_id === data?.userId ? <div className="space-y-2 border-t pt-4"><p className="text-sm text-muted-foreground">Confirme se o atendimento foi concluído conforme esperado.</p><Button onClick={() => conclude.mutate()} disabled={conclude.isPending}>{conclude.isPending ? "Concluindo..." : "Marcar como concluído"}</Button></div> : null}
    <div className="border-t pt-4"><p className="mb-2 flex items-center gap-2 font-semibold"><History className="h-4 w-4" />Histórico</p><div className="space-y-2">{[{ id: `opened-${call.id}`, created_at: call.created_at, text: "Chamado aberto" }, ...(details.data?.logs ?? []).map((log) => ({ id: log.id, created_at: log.created_at, text: log.previous_status !== log.new_status ? `${STATUSES[log.previous_status as CallStatus] ?? "—"} → ${STATUSES[log.new_status as CallStatus] ?? "—"}` : "Responsável ou observação atualizados" })), ...(details.data?.messages ?? []).map((entry) => ({ id: entry.id, created_at: entry.created_at, text: `Mensagem enviada por ${names.get(entry.author_id) ?? "Usuário"}` }))].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((entry) => <div key={entry.id} className="rounded-md bg-muted p-2 text-xs">{entry.text}<span className="ml-2 text-muted-foreground">{new Date(entry.created_at).toLocaleString("pt-BR")}</span></div>)}</div></div>
