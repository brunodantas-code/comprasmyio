...
function VisitDetails({ visit, data, onClose, onChanged }: { visit: Visit | null; data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onClose: () => void; onChanged: () => void }) {
  ...
  const { data: detail } = useQuery({ queryKey: ["site-survey-detail", visit?.id], enabled: Boolean(visit), queryFn: async () => { const [{ data: responses }, { data: attachments }, { data: logs }] = await Promise.all([supabase.from("site_survey_responses").select("*").eq("visit_id", visit?.id ?? ""), supabase.from("site_survey_attachments").select("*").eq("visit_id", visit?.id ?? "").order("created_at"), supabase.from("site_survey_logs").select("*").eq("visit_id", visit?.id ?? "").order("created_at", { ascending: false })]); return { responses: responses ?? [], attachments: attachments ?? [], logs: logs ?? [] }; } });
...
function SurveyProfilesAdmin() {
  const qc = useQueryClient(); const { data = [] } = useQuery({ queryKey: ["site-survey-profiles"], queryFn: async () => { const { data, error } = await supabase.from("site_survey_access_profiles").select("*,site_survey_profile_permissions(permission_key,allowed)").order("name"); if (error) throw error; return data; } });
  ...
  return <Card><CardHeader className="flex flex-row items-start justify-between"><div><CardTitle>Perfis de acesso</CardTitle><CardDescription>Configure menus e ações do Site Survey, no mesmo padrão do Supply.</CardDescription></div><ProfileDialog onSave={save} /></CardHeader><CardContent className="space-y-3">{data.map((profile) => <div key={profile.code} className="flex items-center justify-between gap-3 rounded-md border p-3"><div><p className="font-medium">{profile.name}</p><p className="text-xs text-muted-foreground">{profile.site_survey_profile_permissions.length} acessos selecionados</p></div><ProfileDialog profile={profile} onSave={save} /></div>)}</CardContent></Card>;
}

function ProfileDialog({ profile, onSave }: { profile?: { code: string; name: string; site_survey_profile_permissions: Array<{ permission_key: string; allowed: boolean }> }; onSave: (form: HTMLFormElement, profile?: any) => Promise<void> }) {
  const [open, setOpen] = useState(false); const checked = new Set(profile?.site_survey_profile_permissions.filter((item) => item.allowed).map((item) => item.permission_key) ?? []);
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size={profile ? "compactIcon" : "default"} variant={profile ? "ghost" : "default"} title={profile ? "Editar perfil" : "Novo perfil"}>{profile ? <Pencil className="h-3.5 w-3.5" /> : <><Plus className="h-4 w-4" />Novo perfil</>}</Button></DialogTrigger><DialogContent className="max-w-3xl"><form onSubmit={async (event) => { event.preventDefault(); await onSave(event.currentTarget, profile); setOpen(false); }} className="space-y-5"><DialogHeader><DialogTitle>{profile ? "Editar perfil" : "Novo perfil de acesso"}</DialogTitle><DialogDescription>Escolha as telas e ações permitidas. O Site Survey não possui alçadas nem Approval Workflow.</DialogDescription></DialogHeader><LabeledInput name="name" label="Nome" defaultValue={profile?.name ?? ""} required /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{PERMISSIONS.map(([key, label]) => <label key={key} className="flex items-center gap-2 rounded-md border p-3 text-sm"><Checkbox name={key} defaultChecked={checked.has(key)} />{label}</label>)}</div><DialogFooter><Button type="submit">Salvar perfil</Button></DialogFooter></form></DialogContent></Dialog>;
}
