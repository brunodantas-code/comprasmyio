...
function VisitDetails({ visit, data, onClose, onChanged }: { visit: Visit | null; data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onClose: () => void; onChanged: () => void }) {
  ...
  const generalCallNumbers = new Map(generatedCallsForScope("pre_visit").map((item) => [item.question_id, item.internal_calls?.call_number ?? null]));
  const pointCallNumbers = new Map(generatedCallsForScope("point").map((item) => [item.question_id, item.internal_calls?.call_number ?? null]));
  const phaseForm = (
    ...
    <form onSubmit={(event) => { event.preventDefault(); void saveAnswers(event.currentTarget, "pre_visit").catch((error: unknown) => toast.error(error instanceof Error ? error.message : "Não foi possível salvar o checklist pré-visita.")); }} className="space-y-5">
      {generalSections.map((section, index) => <ChecklistSection key={section.id} section={section} order={index + 1} open={openSectionId === section.id} pending={sectionPending(section, true)} onToggle={() => setOpenSectionId((current) => current === section.id ? null : section.id)}><ConditionalSectionQuestions sectionTitle={section.title} questions={questions.filter((question) => question.section_id === section.id)} answers={answers} attachments={detail?.attachments ?? []} matchesPoint={(item) => !item.visit_luc_id && !item.visit_environment_id} selectedPoint="general" callNumbers={generalCallNumbers} onQuestionAnswerChange={(question, value) => handleQuestionAnswerChange(question, value, "pre_visit")} technicians={data.technicians} visitTechnicians={visitTechnicians} onTechniciansChange={setVisitTechnicians} /></ChecklistSection>)}
...
function ConditionalSectionQuestions({ sectionTitle, questions, answers, attachments, matchesPoint, selectedPoint, callNumbers, onQuestionAnswerChange, technicians, visitTechnicians, onTechniciansChange }: { sectionTitle: string; questions: Question[]; answers: Map<string, unknown>; attachments: Array<{ question_id: string | null; visit_luc_id?: string | null; visit_environment_id?: string | null }>; matchesPoint: (item: { visit_luc_id?: string | null; visit_environment_id?: string | null }) => boolean; selectedPoint: string; callNumbers: Map<string, string | null>; onQuestionAnswerChange: (question: Question, value: unknown) => void; technicians?: Profile[]; visitTechnicians?: VisitTechnician[]; onTechniciansChange?: (rows: VisitTechnician[]) => void }) {
...
function QuestionField({ question, answer, hasPhoto, callNumber, onAnswerChange }: { question: Question; answer: unknown; hasPhoto: boolean; callNumber?: string | null; onAnswerChange?: (value: unknown) => void }) {
  ...
  return <div className={`grid gap-3 py-1 ${showDetail || (config.photo && conditionApplies) ? "lg:grid-cols-[minmax(0,1.35fr)_minmax(260px,0.65fr)] lg:items-start lg:gap-5" : ""}`}>
    <div className="min-w-0 space-y-2">
      <div className="flex min-w-0 flex-wrap items-center gap-2"><Label className="text-sm font-semibold">{question.prompt}{showRequiredMark ? " *" : ""}</Label>{callNumber ? <Badge variant="outline" className="border-myio-green/30 bg-myio-green/10 text-myio-green">Chamado #{callNumber}</Badge> : null}</div>
