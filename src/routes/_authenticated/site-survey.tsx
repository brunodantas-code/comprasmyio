...
function ChecklistAdmin({ data, onChanged }: { data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onChanged: () => void }) {
 ...
 return <div className="grid gap-6 lg:grid-cols-[320px_1fr]"><Card><CardHeader><CardTitle>Modelos</CardTitle><CardDescription>Cadastre e edite os roteiros de vistoria.</CardDescription></CardHeader><CardContent className="space-y-4"><form onSubmit={(event) => { event.preventDefault(); void createTemplate(event.currentTarget); }} className="space-y-3"><LabeledInput name="name" label="Nome do modelo" required /><div className="space-y-2"><Label>Descrição</Label><Textarea name="description" rows={2} /></div><Button type="submit" size="sm"><Plus className="h-4 w-4" />Criar modelo</Button></form><div className="space-y-1 border-t pt-3">{activeTemplates.map((template) => <Button key={template.id} variant={selected === template.id ? "secondary" : "ghost"} className="w-full justify-between" onClick={() => setSelected(template.id)}><span>{template.name}</span>{template.is_default ? <Badge>Padrão</Badge> : null}</Button>)}</div></CardContent></Card><div className="space-y-5">{selectedTemplate ? <div key={selectedTemplate.id} className="space-y-5"><Card><CardHeader><CardTitle>Editar modelo</CardTitle></CardHeader><CardContent><form onSubmit={(event) => { event.preventDefault(); void updateTemplate(event.currentTarget, selectedTemplate.id); }} className="grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]"><LabeledInput name="name" label="Nome" defaultValue={selectedTemplate.name} required /><LabeledInput name="description" label="Descrição" defaultValue={selectedTemplate.description ?? ""} /><Button type="submit" className="self-end"><Pencil className="h-4 w-4" />Salvar</Button><div className="self-end"><ConfirmDeleteButton title={`Excluir ${selectedTemplate.name}?`} description="O modelo deixará de aparecer em novas OS. O histórico existente será preservado." onConfirm={() => deactivate("site_survey_templates", selectedTemplate.id)} /></div></form></CardContent></Card><Card><CardHeader><CardTitle>Seções do checklist</CardTitle><CardDescription>Arraste os blocos pelo marcador para reorganizar a sequência.</CardDescription></CardHeader><CardContent><form onSubmit={(event) => { event.preventDefault(); void createSection(event.currentTarget); }} className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><LabeledInput name="title" label="Título da seção" required /><LabeledInput name="description" label="Descrição" /><Button type="submit" className="self-end"><Plus className="h-4 w-4" />Adicionar</Button></form></CardContent></Card><DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(event) => void reorderSections(event)}><SortableContext items={sectionOrder} strategy={verticalListSortingStrategy}><div className="space-y-5">{sectionOrder.map((sectionId, sectionIndex) => { const section = sectionsById.get(sectionId); if (!section) return null; const questionIds = questionOrders[section.id] ?? []; return <SortableSectionCard key={section.id} id={section.id} order={sectionIndex + 1}><CardHeader className="bg-myio-green/10"><form onSubmit={(event) => { event.preventDefault(); void updateSection(event.currentTarget, section.id); }} className="grid gap-3 sm:grid-cols-[auto_1fr_1fr_auto_auto_auto]"><div className="space-y-2"><Label className="block pl-7 text-left text-lg font-bold">Seção</Label><OrderHandle order={sectionIndex + 1} label={`Mover seção ${section.title}`} compact /></div><div className="space-y-2"><Label htmlFor={`section-title-${section.id}`} className="invisible" aria-hidden="true">Título</Label><Input id={`section-title-${section.id}`} name="title" defaultValue={section.title} required aria-label="Título da seção" /></div><LabeledInput name="description" label="Descrição" defaultValue={section.description ?? ""} /><Button type="submit" size="compactIcon" variant="outline" className="self-end" aria-label="Salvar seção" title="Salvar seção"><Save className="h-3.5 w-3.5" /></Button><div className="self-end"><ConfirmDeleteButton title={`Excluir ${section.title}?`} description="A seção e suas perguntas deixarão de aparecer em novas OS." onConfirm={() => deactivate("site_survey_sections", section.id)} /></div><Button type="button" size="compactIcon" variant="ghost" className="self-end text-myio-green hover:text-myio-green" aria-label={`${collapsedSectionIds.has(section.id) ? "Exibir" : "Recolher"} perguntas da seção ${section.title}`} title={collapsedSectionIds.has(section.id) ? "Exibir perguntas" : "Recolher perguntas"} onClick={() => setCollapsedSectionIds((current) => { const next = new Set(current); if (next.has(section.id)) next.delete(section.id); else next.add(section.id); return next; })}>{collapsedSectionIds.has(section.id) ? <Plus className="h-5 w-5" strokeWidth={2.5} /> : <Minus className="h-5 w-5" strokeWidth={2.5} />}</Button></form></CardHeader>{collapsedSectionIds.has(section.id) ? null : <CardContent className="space-y-4 border-t border-border p-0 sm:p-0"><div className="border-b border-border px-10 py-3 sm:px-8"><h3 className="text-left text-lg font-bold">Perguntas</h3></div><DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(event) => void reorderQuestions(section.id, event)}><SortableContext items={questionIds} strategy={verticalListSortingStrategy}><div id={`question-drop-zone-${section.id}`} className="divide-y divide-border">{questionIds.map((questionId, questionIndex) => { const question = questionsById.get(questionId); if (!question) return null; return <SortableQuestionRow key={question.id} id={question.id}><form onSubmit={(event) => { event.preventDefault(); void updateQuestion(event.currentTarget, question); }} className="grid items-start gap-2 p-3 sm:grid-cols-[auto_minmax(0,1.4fr)_minmax(0,1fr)_auto] sm:p-4"><OrderHandle order={questionIndex + 1} label={`Mover pergunta ${question.prompt}`} compact /><Input className="h-10 min-w-0" name="prompt" defaultValue={question.prompt} required aria-label={`Texto da pergunta ${questionIndex + 1}`} placeholder="Digite a pergunta" /><Input className="h-10 min-w-0" name="options" defaultValue={(Array.isArray(question.options) ? question.options : []).join("; ")} placeholder="Opções separadas por ponto e vírgula" aria-label={`Opções da pergunta ${questionIndex + 1}`} /><div className="flex h-10 items-start justify-end gap-2"><label className="flex h-10 shrink-0 items-center gap-2 text-sm"><Checkbox name="required" defaultChecked={question.required} /><span>{questionRequiresPhoto(question) ? "Foto obrigatória" : "Obrigatória"}</span></label><Button type="submit" size="compactIcon" variant="outline" className="h-10 w-10" aria-label="Salvar pergunta" title="Salvar pergunta"><Save className="h-3.5 w-3.5" /></Button><ConfirmDeleteButton title="Excluir pergunta?" description="A pergunta deixará de aparecer em novas OS; respostas anteriores serão preservadas." onConfirm={() => deactivate("site_survey_questions", question.id)} /></div></form></SortableQuestionRow>; })}</div></SortableContext></DndContext>{addingQuestionSectionId === section.id ? <form onSubmit={(event) => { event.preventDefault(); void createQuestion(event.currentTarget, section.id); }} className="grid items-end gap-3 border-t border-border p-4 sm:grid-cols-[1fr_1fr] sm:p-6 lg:grid-cols-[1fr_1fr_1fr_auto]"><LabeledInput name="prompt" label="Nova pergunta" required /><div className="space-y-2"><Label>Tipo de resposta</Label><Select name="question_type" defaultValue="radio"><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="checkbox">Caixa de seleção</SelectItem><SelectItem value="radio">Escolha única</SelectItem><SelectItem value="multiselect">Múltipla escolha</SelectItem><SelectItem value="text">Texto curto</SelectItem><SelectItem value="textarea">Texto longo</SelectItem><SelectItem value="number">Número</SelectItem><SelectItem value="select">Lista de opções</SelectItem></SelectContent></Select></div><LabeledInput name="options" label="Opções" placeholder="Ex.: Sim; Não; Não se aplica" /><div className="flex items-center gap-3 pb-1"><label className="flex items-center gap-2 text-sm"><Checkbox name="required" />Obrigatória</label><Button type="submit" size="compactIcon" variant="ghost" className="text-myio-green hover:text-myio-green" aria-label="Adicionar pergunta" title="Adicionar pergunta"><Plus className="h-5 w-5" strokeWidth={2.5} /></Button></div></form> : <div className="flex justify-start border-t border-border p-3 sm:px-6"><Button type="button" size="compactIcon" variant="ghost" className="text-myio-green hover:text-myio-green" aria-label="Adicionar pergunta" title="Adicionar pergunta" onClick={() => setAddingQuestionSectionId(section.id)}><Plus className="h-5 w-5" strokeWidth={2.5} /></Button></div>}</CardContent>}</SortableSectionCard>; })}</div></SortableContext></DndContext></div> : <div className="rounded-md border border-dashed p-10 text-center text-muted-foreground">Crie ou selecione um modelo.</div>}</div></div>;
}
