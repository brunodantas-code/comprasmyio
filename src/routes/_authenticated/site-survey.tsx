...
function QuestionPromptLine({ name, defaultValue = "", label, readOnly = false, ariaLabel }: { name: string; defaultValue?: string; label?: string; readOnly?: boolean; ariaLabel?: string }) {
  const [value, setValue] = useState(defaultValue.slice(0, 50));
  return <div className="min-w-0 space-y-1">{label ? <Label>{label}</Label> : null}<Input name={name} value={value} onChange={(event) => setValue(event.target.value.slice(0, 50))} maxLength={50} required readOnly={readOnly} aria-label={ariaLabel ?? label ?? "Texto da pergunta"} placeholder="Digite a pergunta" className={`h-10 min-w-0 rounded-none border-0 border-b bg-transparent px-0 shadow-none focus-visible:ring-0 ${value ? "border-myio-green focus-visible:border-myio-green" : "border-border focus-visible:border-myio-green"}`} /><div className="flex min-h-8 items-start justify-between gap-2 text-[11px] leading-tight text-muted-foreground"><span>Redija a pergunta ou frase do check List.</span><span className="shrink-0 tabular-nums">{value.length}/50</span></div></div>;
}
