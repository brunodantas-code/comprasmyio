...
function VisitDetails({ visit, data, onClose, onChanged }: { visit: Visit | null; data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onClose: () => void; onChanged: () => void }) {
  ...
  const saveAnswers = async (form: HTMLFormElement) => {
    ...
    const rows = questions.map((question) => {
      ...
      const questionScope = generalQuestionIds.has(question.id) ? { visit_luc_id: null, visit_environment_id: null } : scope;
      return { visit_id: visit.id, ...questionScope, question_id: question.id, answered_by: data.userId, answer: config.detail ? { value, detail: detail || null } : value, question_snapshot: JSON.parse(JSON.stringify({ prompt: question.prompt, options: question.options, configuration: question.configuration })) };
    });
    ...
    for (const question of questions.filter((item) => item.question_key !== "shopping_maintenance_companions")) {
      ...
      const isGeneralQuestion = generalQuestionIds.has(question.id);
