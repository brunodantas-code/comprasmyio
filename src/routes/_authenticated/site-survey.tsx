          ...
          <Tabs value={visitsSection} onValueChange={setVisitsSection}>
            ...
            <TabsContent value="minhas">
              ...
              <div className="mb-4 flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar visita, cliente, projeto ou técnico" className="pl-9" /></div><Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="sm:w-52"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="todos">Todas as situações</SelectItem>{STATUS_ORDER.map((status) => <SelectItem key={status} value={status}>{STATUS[status]}</SelectItem>)}</SelectContent></Select></div>
               <div className="overflow-hidden rounded-md border border-border bg-card"><div className="hidden grid-cols-[250px_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_150px_70px] gap-3 border-b bg-primary/20 px-4 py-3 text-sm font-semibold lg:grid"><span>Nº / Abrir</span><span>Cliente / Projeto</span><span>Técnico</span><span>Agendamento</span><span>Situação</span><span>Ações</span></div>{filtered.map((visit) => <div key={visit.id} className="grid gap-3 border-b border-border px-4 py-4 last:border-0 lg:grid-cols-[250px_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_150px_70px] lg:items-center"><button type="button" className="w-fit whitespace-nowrap text-left text-sm font-semibold text-foreground underline decoration-myio-green decoration-2 underline-offset-4 hover:text-myio-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`Exibir visita ${String(visit.survey_number).padStart(12, "0")}`} onClick={() => setSelected((current) => current?.id === visit.id ? null : visit)}>{String(visit.survey_number).padStart(12, "0")}</button><div className="min-w-0"><p className="truncate text-sm font-medium">{names.clients.get(visit.client_id ?? "") ?? names.projects.get(visit.project_id ?? "") ?? "—"}</p>{visit.client_id && visit.project_id ? <p className="truncate text-xs text-muted-foreground">{names.projects.get(visit.project_id)}</p> : null}</div><span className="truncate text-sm">{names.technicians.get(visit.technician_id) ?? "—"}</span><div className="text-sm"><p>{new Date(visit.scheduled_start).toLocaleDateString("pt-BR")}</p><p className="text-xs text-muted-foreground">{new Date(visit.scheduled_start).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p></div><Badge variant="status" className="w-fit">{STATUS[visit.status]}</Badge><div className="flex gap-1">{can("site_survey_editar") ? <VisitDialog data={data} visit={visit} onSaved={invalidate} /> : null}{can("site_survey_editar") && visit.status !== "cancelada" && visit.status !== "concluida" ? <ConfirmDeleteButton title="Cancelar visita?" description="Confirme o cancelamento desta visita. O histórico da OS será mantido." ariaLabel="Cancelar visita" confirmLabel="Confirmar cancelamento" pendingLabel="Cancelando..." onConfirm={() => void cancelVisit(visit)} trigger={<Button type="button" size="compactIcon" variant="ghost" title="Cancelar visita" aria-label="Cancelar visita"><X className="h-3.5 w-3.5" /></Button>} /> : null}{can("site_survey_excluir") ? <ConfirmDeleteButton title="Excluir visita?" description="Esta ação excluirá definitivamente a visita, respostas, fotos e histórico" confirmLabel="Excluir" equalActions onConfirm={async () => { const { error } = await supabase.from("site_survey_visits").delete().eq("id", visit.id); if (error) return toast.error(error.message); toast.success("Visita excluída"); invalidate(); }} /> : null}</div></div>)}{filtered.length === 0 ? <p className="px-4 py-12 text-center text-sm text-muted-foreground">Nenhuma visita encontrada.</p> : null}</div>
...
function VisitDetails({ visit, data, onClose, onChanged }: { visit: Visit | null; data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onClose: () => void; onChanged: () => void }) {
  ...
  const generalSections = sections.slice(0, 3);
  const pointSections = sections.slice(3);
  ...
  const pointPendingFields = (form: HTMLFormElement) => {
    ...
    for (const section of pointSections) {
      if (section.title === "Materiais necessários") {
        if (!noAdditionalMaterial && !materialRows.some((item) => item.catalog_item_id && Number(item.quantity) > 0)) pending.push(`${section.title}: informe os materiais ou marque que não são necessários`);
        continue;
      }
  ...
  const saveAnswers = async (form: HTMLFormElement, phase: "pre_visit" | "point", finishPoint = false, acceptedPendingFields: string[] = []) => {
    ...
    if (phase === "pre_visit") {
      ...
      for (const file of files) { const safe = file.name.replace(/[^a-zA-Z0-9._-]/g, "-"); const path = `${visit.id}/${pointId}/${crypto.randomUUID()}-${safe}`; const { error: uploadError } = await supabase.storage.from("site-survey-attachments").upload(path, file); if (uploadError) throw uploadError; const { error } = await supabase.from("site_survey_attachments").insert({ visit_id: visit.id, ...scope, uploaded_by: data.userId, file_name: file.name, storage_path: path, content_type: file.type, file_size: file.size }); if (error) throw error; }
       ...
       const otherPointsComplete = points.filter((point) => point.value !== selectedPoint).every((point) => point.completionStatus === "concluida");
  ...
  const storedSpecialEquipmentAnswer = specialEquipmentQuestion ? String(answerParts(answers.get(specialEquipmentQuestion.id)).value ?? "") : "";
  ...
  const phaseForm = (
    ...
    <form onSubmit={(event) => { event.preventDefault(); void saveAnswers(event.currentTarget, "pre_visit").catch((error: unknown) => toast.error(error instanceof Error ? error.message : "Não foi possível salvar o checklist pré-visita.")); }} className="space-y-5">
      {generalSections.map((section, index) => <ChecklistSection key={section.id} section={section} order={index + 1} open={openSectionId === section.id} pending={sectionPending(section, true)} onToggle={() => setOpenSectionId((current) => current === section.id ? null : section.id)}><ConditionalSectionQuestions sectionTitle={section.title} questions={questions.filter((question) => question.section_id === section.id)} answers={answers} attachments={detail?.attachments ?? []} matchesPoint={(item) => !item.visit_luc_id && !item.visit_environment_id} selectedPoint="general" calls={generalCalls} onQuestionAnswerChange={(question, value) => handleQuestionAnswerChange(question, value, "pre_visit")} technicians={data.technicians} visitTechnicians={visitTechnicians} onTechniciansChange={setVisitTechnicians} /></ChecklistSection>)}
    ...
    <form onSubmit={(event) => { event.preventDefault(); void saveAnswers(event.currentTarget, "point").catch((error: unknown) => toast.error(error instanceof Error ? error.message : "Não foi possível salvar a vistoria.")); }} className="space-y-5">
       ...
       {selectedPoint ? pointSections.map((section, index) => <ChecklistSection key={`${selectedPoint}-${section.id}`} section={section} order={generalSections.length + index + 1} open={openSectionId === section.id} pending={sectionPending(section, false) || (section.title.startsWith("Revisão") && selectedPointRecord?.completion_status !== "concluida")} onToggle={() => setOpenSectionId((current) => current === section.id ? null : section.id)}>{section.title.startsWith("Revisão") && displayedPendingFields.length ? <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3"><p className="text-sm font-semibold text-destructive">Pendências {pendingPointSave ? "encontradas" : "do último salvamento"}</p><ul className="mt-2 space-y-1 text-sm text-destructive">{displayedPendingFields.map((field) => <li key={field}>• {field}</li>)}</ul></div> : null}<ConditionalSectionQuestions sectionTitle={section.title} questions={questions.filter((question) => question.section_id === section.id)} answers={answers} attachments={detail?.attachments ?? []} matchesPoint={matchesPoint} selectedPoint={selectedPoint} calls={pointCalls} onQuestionAnswerChange={(question, value) => { if (isSpecialEquipmentQuestion(question)) setSpecialEquipmentAnswer(String(value)); handleQuestionAnswerChange(question, value, "point"); }} />{section.title === "Materiais necessários" ? renderMaterials : null}</ChecklistSection>) : null}
...
function QuestionField({ question, answer, hasPhoto, call, onAnswerChange }: { question: Question; answer: unknown; hasPhoto: boolean; call?: { id: string | null; number: string | null }; onAnswerChange?: (value: unknown) => void }) {
