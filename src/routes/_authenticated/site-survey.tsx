...
function LineField({ name, label, defaultValue = "", placeholder, required = false, id, ariaLabel, hideLabel = false, active = false, sectionAligned = false, maxLength, showCounter = false, compact = false }: { name: string; label: string; defaultValue?: string; placeholder?: string; required?: boolean; id?: string; ariaLabel?: string; hideLabel?: boolean; active?: boolean; sectionAligned?: boolean; maxLength?: number; showCounter?: boolean; compact?: boolean }) {
  ...
  return <div className={`min-w-0 ${sectionAligned ? "space-y-4" : compact ? "space-y-1" : "space-y-2"}`}><Label htmlFor={id} className={hideLabel ? "invisible" : undefined} aria-hidden={hideLabel || undefined}>{label}</Label><Input id={id} name={name} value={value} onChange={(event) => setValue(event.target.value.slice(0, maxLength))} maxLength={maxLength} placeholder={placeholder} required={required} aria-label={ariaLabel ?? label} className={`h-10 min-w-0 rounded-none border-0 border-b bg-transparent px-0 shadow-none focus-visible:border-myio-green focus-visible:ring-0 ${active ? "border-myio-green" : "border-border"}`} />{showCounter && maxLength ? <div className="flex h-4 justify-end text-[11px] text-muted-foreground"><span className="tabular-nums">{value.length}/{maxLength}</span></div> : <div className={compact ? "h-4" : "h-8"} aria-hidden="true" />}</div>;
}
