    ...
    <nav className="fixed inset-x-3 bottom-3 z-50 sm:hidden" aria-label="Navegação do Site Survey">
      <div className="mx-auto grid max-w-md grid-cols-4 items-stretch rounded-[1.75rem] border border-border bg-card/95 p-1.5 shadow-xl backdrop-blur supports-[backdrop-filter]:bg-card/90">
        {tabs.map(({ value, label, icon: Icon }) => {
          ...
          const item = <span className={`flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-[1.35rem] px-1 transition-colors ${active ? "bg-primary/15 text-primary" : "text-muted-foreground"}`}><Icon className="h-5 w-5 shrink-0" /><span className="max-w-full truncate text-[10px] font-semibold leading-none">{label}</span></span>;
          if (!hasSubmenu) return <Button key={value} type="button" variant="ghost" className="h-auto min-w-0 rounded-[1.35rem] p-0 hover:bg-transparent" onClick={selectSection} aria-current={active ? "page" : undefined}>{item}</Button>;
          const submenuItems = value === "visitas"
            ...
            : userTabs;
          return <DropdownMenu key={value}><DropdownMenuTrigger asChild><Button type="button" variant="ghost" className="h-auto min-w-0 rounded-[1.35rem] p-0 hover:bg-transparent" onClick={selectSection} aria-current={active ? "page" : undefined}>{item}</Button></DropdownMenuTrigger><DropdownMenuContent side="top" align="center" sideOffset={10} className="w-56 rounded-lg p-1.5 shadow-xl"><DropdownMenuLabel>{label}</DropdownMenuLabel><DropdownMenuSeparator />{submenuItems.map(({ value: subValue, label: subLabel, icon: SubIcon }) => { const subActive = value === "visitas" ? visitsSection === subValue : userSection === subValue; return <DropdownMenuItem key={subValue} className="min-h-11 rounded-md" onSelect={() => { selectSection(); if (value === "visitas") setVisitsSection(subValue); else setUserSection(subValue); }}><SubIcon className="h-4 w-4" /><span className="flex-1">{subLabel}</span>{subActive ? <Check className="h-4 w-4 text-primary" /> : null}</DropdownMenuItem>; })}</DropdownMenuContent></DropdownMenu>;
        })}
      </div>
    </nav>
  </div>;
