...
function VisitDetails({ visit, data, onClose, onChanged }: { visit: Visit | null; data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onClose: () => void; onChanged: () => void }) {
  ...
  const saveAnswers = async (form: HTMLFormElement) => {
    ...
    const rows = visibleQuestions.map((question) => {
      ...
      if (otherApplies && !detail) throw new Error(`Especifique: ${question.prompt}`);
      ...
      return { visit_id: visit.id, ...questionScope, question_id: question.id, answered_by: data.userId, answer: config.detail || config.other_detail ? { value, detail: detail || null } : value, question_snapshot: JSON.parse(JSON.stringify({ prompt: question.prompt, options: question.options, configuration: question.configuration })) };
    });
