...
function ChecklistAdmin({ data, onChanged }: { data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onChanged: () => void }) {
  ...
  const createQuestion = async (form: HTMLFormElement, sectionId: string) => { const values = new FormData(form); const options = String(values.get("options") ?? "").split("\n").map((item) => item.trim()).filter(Boolean); const { error } = await supabase.from("site_survey_questions").insert({ section_id: sectionId, prompt: String(values.get("prompt")), question_type: String(values.get("question_type")), required: values.get("required") === "on", options, position: questionOrders[sectionId]?.length ?? 0 }); if (error) return toast.error(error.message); toast.success("Pergunta adicionada"); form.reset(); onChanged(); };
  ...
  const updateQuestion = async (form: HTMLFormElement, question: Question) => { const values = new FormData(form); const options = String(values.get("options") ?? "").split("\n").map((item) => item.trim()).filter(Boolean); const { error } = await supabase.from("site_survey_questions").update({ prompt: String(values.get("prompt")).trim(), required: values.get("required") === "on", options }).eq("id", question.id); if (error) return toast.error(error.message); toast.success("Pergunta atualizada"); onChanged(); };
  ...
  const reorderQuestions = async (sectionId: string, { active, over }: DragEndEvent) => { if (!over || active.id === over.id) return; const oldOrder = questionOrders[sectionId] ?? []; const from = oldOrder.indexOf(String(active.id)); const to = oldOrder.indexOf(String(over.id)); if (from < 0 || to < 0) return; const next = arrayMove(oldOrder, from, to); setQuestionOrders((current) => ({ ...current, [sectionId]: next })); try { await persistOrder("site_survey_questions", next); toast.success("Perguntas reordenadas"); onChanged(); } catch (error) { setQuestionOrders((current) => ({ ...current, [sectionId]: oldOrder })); toast.error(error instanceof Error ? error.message : "Não foi possível reordenar as perguntas."); } };
...
function SortableQuestionRow({ id, children }: { id: string; children: ReactNode }) {
  const sortable = useSortable({ id });
  return <div ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} className={sortable.isDragging ? "relative z-20 rounded-md bg-card shadow-lg" : undefined}><SortableHandleContext.Provider value={{ attributes: sortable.attributes, listeners: sortable.listeners }}>{children}</SortableHandleContext.Provider></div>;
}
