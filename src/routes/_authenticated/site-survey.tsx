...
function LineField({ name, label, defaultValue = "", placeholder, required = false, id, ariaLabel, hideLabel = false, active = false }: { name: string; label: string; defaultValue?: string; placeholder?: string; required?: boolean; id?: string; ariaLabel?: string; hideLabel?: boolean; active?: boolean }) {
  return <div className="min-w-0 space-y-2"><Label htmlFor={id} className={hideLabel ? "invisible" : undefined} aria-hidden={hideLabel || undefined}>{label}</Label><Input id={id} name={name} defaultValue={defaultValue} placeholder={placeholder} required={required} aria-label={ariaLabel ?? label} className={`h-10 min-w-0 rounded-none border-0 border-b bg-transparent px-0 shadow-none focus-visible:border-myio-green focus-visible:ring-0 ${active ? "border-myio-green" : "border-border"}`} /><div className="h-8" aria-hidden="true" /></div>;
}
