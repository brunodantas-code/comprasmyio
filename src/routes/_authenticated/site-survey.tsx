...
function ChecklistSection({ section, order, open, pending, onToggle, children }: { section: Section; order: number; open: boolean; pending: boolean; onToggle: () => void; children: ReactNode }) {
  return <section className="overflow-hidden rounded-md border border-border bg-card transition-colors">
    <Button type="button" variant="ghost" className="h-auto w-full justify-between rounded-none !bg-card px-4 py-3 text-left !text-foreground hover:!bg-muted/50 hover:!text-foreground" onClick={onToggle} aria-expanded={open}>
      ...
      <span className="flex shrink-0 items-center gap-2">{pending ? <Badge variant="outline" className="border-myio-green/30 bg-myio-green/10 text-[10px] font-semibold uppercase text-myio-green">Pendente</Badge> : null}<span className="flex h-8 w-8 items-center justify-center text-myio-green" aria-hidden="true">{open ? <Minus className="h-5 w-5" strokeWidth={3} /> : <Plus className="h-5 w-5" strokeWidth={3} />}</span></span>
    </Button>
...
function ChecklistAdmin({ data, onChanged }: { data: NonNullable<ReturnType<typeof useSurveyDataShape>>; onChanged: () => void }) {
  ...
  const createTemplate = async (form: HTMLFormElement) => { const values = new FormData(form); const description = String(values.get("description")).trim(); if (description.length > 100) return toast.error("A descrição deve ter no máximo 100 caracteres."); const { error } = await supabase.from("site_survey_templates").insert({ name: String(values.get("name")), description: description || null, created_by: data.userId }); if (error) return toast.error(error.message); toast.success("Modelo criado"); form.reset(); onChanged(); };
  const createSection = async (form: HTMLFormElement) => { if (!selected) return; const values = new FormData(form); const description = String(values.get("description")).trim(); if (description.length > 100) return toast.error("A descrição deve ter no máximo 100 caracteres."); const { error } = await supabase.from("site_survey_sections").insert({ template_id: selected, title: String(values.get("title")), description: description || null, position: sectionOrder.length }); if (error) return toast.error(error.message); toast.success("Seção adicionada"); form.reset(); onChanged(); };
  ...
  const updateTemplate = async (form: HTMLFormElement, id: string) => { const values = new FormData(form); const description = String(values.get("description")).trim(); if (description.length > 100) return toast.error("A descrição deve ter no máximo 100 caracteres."); const { error } = await supabase.from("site_survey_templates").update({ name: String(values.get("name")).trim(), description: description || null }).eq("id", id); if (error) return toast.error(error.message); toast.success("Modelo atualizado"); onChanged(); };
  const updateSection = async (form: HTMLFormElement, id: string) => { const values = new FormData(form); const description = String(values.get("description")).trim(); if (description.length > 100) return toast.error("A descrição deve ter no máximo 100 caracteres."); const { error } = await supabase.from("site_survey_sections").update({ title: String(values.get("title")).trim(), description: description || null }).eq("id", id); if (error) return toast.error(error.message); toast.success("Seção atualizada"); onChanged(); };
...
function LineField({ name, label, defaultValue = "", placeholder, required = false, id, ariaLabel, hideLabel = false, active = false, sectionAligned = false, maxLength, showCounter = false, compact = false }: { name: string; label: string; defaultValue?: string; placeholder?: string; required?: boolean; id?: string; ariaLabel?: string; hideLabel?: boolean; active?: boolean; sectionAligned?: boolean; maxLength?: number; showCounter?: boolean; compact?: boolean }) {
  ...
  return <div onReset={() => setLength(0)} className={`min-w-0 ${sectionAligned ? compact ? "space-y-2" : "space-y-4" : compact ? "space-y-1" : "space-y-2"}`}><Label htmlFor={id} className={hideLabel ? "invisible" : undefined} aria-hidden={hideLabel || undefined}>{label}</Label><Input id={id} name={name} defaultValue={initialValue} onInput={(event) => setLength(event.currentTarget.value.length)} maxLength={maxLength} placeholder={placeholder} required={required} aria-label={ariaLabel ?? label} className={`h-10 min-w-0 rounded-none border-0 border-b bg-transparent px-0 shadow-none focus-visible:border-myio-green focus-visible:ring-0 ${active ? "border-myio-green" : "border-border"}`} />{showCounter && maxLength ? <div className="flex h-4 justify-end text-[11px] text-muted-foreground"><span className="tabular-nums">{length}/{maxLength}</span></div> : <div className={compact ? "h-4" : "h-8"} aria-hidden="true" />}</div>;
}
