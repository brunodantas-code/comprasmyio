               ...
               <div className="overflow-hidden rounded-md border border-border bg-card">
                 ...
                 {filtered.map((visit) => {
                   ...
                   return <div key={visit.id} className={`${visitsListOpen ? "" : "lg:hidden"} grid gap-3 border-b border-border px-4 py-3 last:border-0 lg:grid-cols-[250px_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_150px_70px] lg:items-center lg:py-4`}>
                     <div className="flex items-center justify-between gap-3 lg:block"><button type="button" className="w-fit whitespace-nowrap text-left text-sm font-semibold text-foreground underline decoration-myio-green decoration-2 underline-offset-4 hover:text-myio-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`Exibir visita ${String(visit.survey_number).padStart(12, "0")}`} onClick={() => setSelected((current) => current?.id === visit.id ? null : visit)}>{String(visit.survey_number).padStart(12, "0")}</button><Button type="button" size="compactIcon" variant="ghost" className="lg:hidden" aria-label={visitExpanded ? `Recolher informações da visita ${visit.survey_number}` : `Exibir informações da visita ${visit.survey_number}`} title={visitExpanded ? "Recolher" : "Exibir"} onClick={() => setExpandedVisitIds((current) => { const next = new Set(current); if (next.has(visit.id)) next.delete(visit.id); else next.add(visit.id); return next; })}><ChevronDown className={`h-5 w-5 transition-transform ${visitExpanded ? "rotate-180" : ""}`} /></Button></div>
