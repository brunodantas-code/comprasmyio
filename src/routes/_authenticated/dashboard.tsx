        ...
        <div className="mx-auto grid max-w-2xl grid-cols-5 items-stretch rounded-[1.75rem] border border-border bg-card/95 p-1.5 shadow-xl backdrop-blur supports-[backdrop-filter]:bg-card/90">
          {navigationTabs.map(({ value, label, icon: Icon }) => {
            ...
            const submenuItems = value === "pedidos"
              ...
              : value === "projects"
                ? [{ value: "projetos", label: "Projetos", icon: FolderKanban }, { value: "clientes", label: "Clientes", icon: Building2 }, { value: "centros", label: "Centro de Custo", icon: Landmark }, { value: "cargos", label: "Cargos", icon: Briefcase }, { value: "lembretes", label: "Lembretes", icon: Bell }, { value: "diversos", label: "Diversos", icon: Layers3 }].filter((item) => me.canAccess(`cadastro_${item.value}`))
                : value === "admin"
                  ? [{ value: "usuarios", label: "Usuários", icon: Users, permission: "usuarios_lista" }, { value: "acesso-restrito", label: "Perfis de acesso", icon: ShieldCheck, permission: "usuarios_acesso_restrito" }, { value: "workflow", label: "Approval Workflow", icon: CheckCircle2, permission: "usuarios_workflow" }, { value: "logs", label: "Logs", icon: ScrollText, permission: "usuarios_logs" }, { value: "backup", label: "Backup", icon: DatabaseBackup, permission: "usuarios_backup" }].filter((item) => me.canAccess(item.permission))
                  : [];
