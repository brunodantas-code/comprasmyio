    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex gap-2">
        ...
        <Dialog open={open} onOpenChange={(value) => { setOpen(value); if (!value) setEditing(null); }}>
          <DialogTrigger asChild><Button className="self-end"><Plus className="h-3.5 w-3.5" />Nova conta</Button></DialogTrigger>
      ...
      {isLoading ? <p className="text-sm text-muted-foreground">Carregando...</p> : !rows.length ? <p className="text-sm text-muted-foreground">Cadastre a primeira conta do plano.</p> : <Table className="md:table-fixed">
        ...
        <TableBody>
          ...
          return <TableRow key={account.id} className={!account.active ? "opacity-50" : undefined}>
            ...
            {MONTHS.map(([key, label]) => <TableCell key={key} className="px-1"><MoneyInput aria-label={`${label} de ${account.name}`} value={String(drafts[account.id]?.[key] ?? budget?.[key] ?? 0)} onChange={(value) => setDrafts((old) => ({ ...old, [account.id]: { ...old[account.id], [key]: value } }))} className="h-8 min-w-0 px-1 text-center text-xs" disabled={!account.accepts_entries || children.has(account.id)} /></TableCell>)}
            <TableCell className="px-1 text-center text-xs font-semibold">{BRL.format(total)}</TableCell>
            <TableCell className="px-1"><div className="flex items-center justify-end gap-0"><Button size="compactIcon" variant="ghost" title="Salvar orçamento" aria-label="Salvar orçamento" onClick={() => saveBudget.mutate(account.id)} disabled={!account.accepts_entries || children.has(account.id) || saveBudget.isPending}><Save className="h-4 w-4" /></Button><Button size="compactIcon" variant="ghost" title="Editar conta" aria-label="Editar conta" onClick={() => { setEditing(account); setOpen(true); }}><Pencil className="h-3.5 w-3.5" /></Button><ConfirmDeleteButton title="Excluir conta?" description={`Confirma a exclusão de “${accountLabel(account)}”? Contas com vínculos ou subcontas não poderão ser excluídas.`} ariaLabel={`Excluir conta ${account.name}`} pending={removeAccount.isPending} onConfirm={() => removeAccount.mutate(account.id)} /></div></TableCell>
          </TableRow>;
        })}</TableBody>
      </Table>}
    </CardContent></Card>
