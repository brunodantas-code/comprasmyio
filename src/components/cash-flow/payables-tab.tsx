...
function ClassifyPayable({ payable, accounts, userId, onDone }: { payable: Payable; accounts: Account[]; userId: string; onDone: () => void }) {
  const [open, setOpen] = useState(false); const [accountId, setAccountId] = useState(payable.account_id ?? ""); const [fiscal, setFiscal] = useState(monthValue(payable.fiscal_period)); const [due, setDue] = useState(monthValue(payable.due_date)); const [competence, setCompetence] = useState(monthValue(payable.competence_period)); const [cash, setCash] = useState(monthValue(payable.cash_period));
