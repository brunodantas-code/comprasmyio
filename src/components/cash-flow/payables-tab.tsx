...
function ClassifyPayable({ payable, accounts, userId, onDone }: { payable: Payable; accounts: Account[]; userId: string; onDone: () => void }) {
  ...
  const [open, setOpen] = useState(false); const [accountId, setAccountId] = useState(payable.account_id ?? ""); const [fiscal, setFiscal] = useState(monthValue(payable.fiscal_period)); const [due, setDue] = useState(monthValue(payable.due_date) || currentPeriod); const [competence, setCompetence] = useState(monthValue(payable.competence_period)); const [cash, setCash] = useState(monthValue(payable.cash_period));
  const save = useMutation({ mutationFn: async () => { if (!accountId || !fiscal || !due || !competence || !cash) throw new Error("Preencha o Plano de Contas e todas as referências mensais."); const { error } = await supabase.from("cash_flow_payables").update({ account_id: accountId, fiscal_period: periodDate(fiscal), due_date: periodDate(due), competence_period: periodDate(competence), cash_period: periodDate(cash), status: payable.status === "a_classificar" ? "a_pagar" : payable.status, classified_by: userId, classified_at: new Date().toISOString() }).eq("id", payable.id); if (error) throw error; }, onSuccess: () => { toast.success("Classificação salva"); setOpen(false); onDone(); }, onError: (error: Error) => toast.error(error.message) });
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="ghost" className="h-auto p-0 hover:bg-transparent" title="Classificar" aria-label="Classificar"><Badge variant="status" className="cursor-pointer">Classificar</Badge></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Classificar pagamento</DialogTitle></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label>Plano de Contas</Label><AccountPicker accounts={accounts} value={accountId} onValueChange={setAccountId} /></div><PeriodSelect label="Emissão fiscal" value={fiscal} onValueChange={setFiscal} /><PeriodSelect label="Vencimento Original" value={due} onValueChange={setDue} /><PeriodSelect label="Competência" value={competence} onValueChange={setCompetence} /><PeriodSelect label="Caixa (Previsão)" value={cash} onValueChange={setCash} /></div><Button onClick={() => save.mutate()} disabled={save.isPending}>Salvar classificação</Button></DialogContent></Dialog>;
}
