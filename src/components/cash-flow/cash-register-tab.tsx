...
import { Building2, Link2, Pencil, Upload } from "lucide-react";
...
import { Account, BankAccount, BRL, accountLabel, formatDate, formatPeriod, isPostingAccount, monthValue, periodDate } from "./shared";
...
type Transaction = PreviewRow & { id: string; bank_account_id: string; reconciliation_status: string; account_id: string | null; fiscal_period: string; competence_period: string; cash_period: string };
...
export function CashRegisterTab({ userId }: { userId: string }) {
  ...
  const { data: chartAccounts = [] } = useCashFlowAccounts();
  const postingAccounts = useMemo(() => chartAccounts.filter((item) => isPostingAccount(item, chartAccounts)), [chartAccounts]);
  ...
  const { data: transactions = [], isLoading } = useQuery({ queryKey: ["cash-flow-transactions"], queryFn: async () => { const { data, error } = await supabase.from("cash_flow_transactions").select("id,bank_account_id,external_id,posted_at,amount,description,reconciliation_status,account_id,fiscal_period,competence_period,cash_period").order("posted_at", { ascending: false }); if (error) throw error; return data as Transaction[]; } });
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader className="gap-3 sm:flex-row sm:items-end sm:justify-between"><div><CardTitle>Movimentações bancárias</CardTitle><CardDescription>Entradas e saídas importadas de OFX ou CSV.</CardDescription></div><div className="w-full sm:w-60"><Select value={selectedAccount} onValueChange={setSelectedAccount}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas as contas</SelectItem>{accounts.map((account) => <SelectItem key={account.id} value={account.id}>{account.name}</SelectItem>)}</SelectContent></Select></div></CardHeader><CardContent>
      {isLoading ? <p className="text-sm text-muted-foreground">Carregando...</p> : !visible.length ? <p className="text-sm text-muted-foreground">Nenhuma movimentação importada.</p> : <Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Descrição</TableHead><TableHead>Conta bancária</TableHead><TableHead>Plano de Contas</TableHead><TableHead>Referências</TableHead><TableHead className="text-right">Valor</TableHead><TableHead>Situação</TableHead><TableHead aria-label="Ações" /></TableRow></TableHeader><TableBody>{visible.map((transaction) => { const bank = accounts.find((item) => item.id === transaction.bank_account_id); return <TableRow key={transaction.id}><TableCell>{formatDate(transaction.posted_at)}</TableCell><TableCell className="max-w-64 whitespace-normal">{transaction.description}</TableCell><TableCell>{bank?.name ?? "—"}</TableCell><TableCell>{chartAccounts.find((item) => item.id === transaction.account_id)?.name ?? "A classificar"}</TableCell><TableCell className="min-w-32 text-xs"><p>Emissão: {formatPeriod(transaction.fiscal_period)}</p><p>Competência: {formatPeriod(transaction.competence_period)}</p><p>Caixa: {formatPeriod(transaction.cash_period)}</p></TableCell><TableCell className={`text-right font-semibold ${transaction.amount < 0 ? "text-destructive" : "text-primary"}`}>{BRL.format(Number(transaction.amount))}</TableCell><TableCell><Badge variant="status">{reconciliationLabel[transaction.reconciliation_status] ?? transaction.reconciliation_status}</Badge></TableCell><TableCell><div className="flex items-center gap-1"><ClassifyTransaction transaction={transaction} accounts={postingAccounts} onDone={() => qc.invalidateQueries({ queryKey: ["cash-flow-transactions"] })} />{transaction.amount < 0 && transaction.reconciliation_status !== "conciliado" ? <ReconcileDialog transaction={transaction} payables={payables} userId={userId} /> : null}</div></TableCell></TableRow>; })}</TableBody></Table>}
    </CardContent></Card>
  </div>;
}
...
function ClassifyTransaction({ transaction, accounts, onDone }: { transaction: Transaction; accounts: Account[]; onDone: () => void }) {
  const [open, setOpen] = useState(false); const [accountId, setAccountId] = useState(transaction.account_id ?? ""); const [fiscal, setFiscal] = useState(monthValue(transaction.fiscal_period)); const [competence, setCompetence] = useState(monthValue(transaction.competence_period));
  const save = useMutation({ mutationFn: async () => { if (!accountId || !fiscal || !competence) throw new Error("Preencha o Plano de Contas, Emissão e Competência."); const { error } = await supabase.from("cash_flow_transactions").update({ account_id: accountId, fiscal_period: periodDate(fiscal), competence_period: periodDate(competence) }).eq("id", transaction.id); if (error) throw error; }, onSuccess: () => { toast.success("Movimentação classificada"); setOpen(false); onDone(); }, onError: (error: Error) => toast.error(error.message) });
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="compactIcon" variant="ghost" title="Classificar movimentação" aria-label="Classificar movimentação"><Pencil className="h-3.5 w-3.5" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Classificar movimentação</DialogTitle><DialogDescription>A referência de Caixa é definida pelo mês real do extrato.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label>Plano de Contas</Label><Select value={accountId} onValueChange={setAccountId}><SelectTrigger><SelectValue placeholder="Selecionar conta" /></SelectTrigger><SelectContent>{accounts.map((account) => <SelectItem key={account.id} value={account.id}>{accountLabel(account)}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Emissão fiscal</Label><Input type="month" value={fiscal} onChange={(event) => setFiscal(event.target.value)} required /></div><div className="space-y-2"><Label>Competência</Label><Input type="month" value={competence} onChange={(event) => setCompetence(event.target.value)} required /></div><div className="space-y-2"><Label>Caixa</Label><Input type="month" value={monthValue(transaction.cash_period)} disabled /></div></div><DialogFooter><Button onClick={() => save.mutate()} disabled={save.isPending}>Salvar classificação</Button></DialogFooter></DialogContent></Dialog>;
}
