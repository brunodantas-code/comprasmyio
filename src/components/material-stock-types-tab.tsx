...
export function MaterialStockTypesTab() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { toast.success("Tipo de estoque criado"); setCreateOpen(false); invalidate(); },
  ...
  return <Collapsible open={expanded} onOpenChange={setExpanded} asChild><Card>
    <CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle>Tipos de Estoque</CardTitle>{expanded && <CardDescription>Os tipos ativos aparecem ao cadastrar um item novo em Materiais.</CardDescription>}</div><div className="flex items-center gap-1"><CollapsibleContent><StockTypeDialog title="Novo tipo de estoque" defaultPosition={Math.max(0, ...(types ?? []).map((item) => item.position)) + 1} saving={create.isPending} onSave={(name, position, destinationType) => create.mutateAsync({ name, position, destinationType })} trigger={<Button size="compactIcon" aria-label="Criar tipo de estoque" title="Criar tipo"><Plus className="h-3.5 w-3.5" /></Button>} open={createOpen} onOpenChange={setCreateOpen} /></CollapsibleContent><CollapsibleTrigger asChild><Button size="compactIcon" variant="ghost" aria-label={expanded ? "Recolher Tipos de Estoque" : "Expandir Tipos de Estoque"} title={expanded ? "Recolher" : "Expandir"}>{expanded ? <Minus className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}</Button></CollapsibleTrigger></div></CardHeader>
...
function StockTypeDialog({ title = "Editar tipo de estoque", defaultName = "", defaultPosition, saving, onSave, trigger, open: controlledOpen, onOpenChange }: { title?: string; defaultName?: string; defaultPosition: number; saving: boolean; onSave: (name: string, position: number, destinationType: MaterialStockType["destination_type"]) => Promise<unknown>; trigger: ReactNode; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (next) { setName(defaultName); setPosition(String(defaultPosition)); setDestinationType("fabrica"); } }}><DialogTrigger asChild>{trigger}</DialogTrigger><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>Defina o nome, a ordem e a área onde os itens serão cadastrados.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={async (event) => { event.preventDefault(); const order = Number(position); if (name.trim().length < 2 || name.trim().length > 120) return toast.error("Informe um nome entre 2 e 120 caracteres"); if (!Number.isInteger(order) || order < 0) return toast.error("Informe uma ordem válida"); try { await onSave(name.trim(), order, destinationType); setOpen(false); } catch { /* A alteração exibe a mensagem. */ } }}><div className="space-y-2"><Label htmlFor={`material-stock-type-name-${defaultName || "new"}`}>Nome</Label><Input id={`material-stock-type-name-${defaultName || "new"}`} value={name} onChange={(event) => setName(event.target.value)} maxLength={120} required /></div>{!defaultName ? <div className="space-y-2"><Label>Área de destino</Label><Select value={destinationType} onValueChange={(value) => setDestinationType(value as MaterialStockType["destination_type"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="fabrica">Estoque Fábrica</SelectItem><SelectItem value="terceiros">Estoque Myio</SelectItem><SelectItem value="almoxarifado">Estoque Almoxarifado</SelectItem><SelectItem value="ferramentas">Ferramentas e Ativos</SelectItem></SelectContent></Select></div> : null}<div className="space-y-2"><Label htmlFor={`material-stock-type-position-${defaultName || "new"}`}>Ordem</Label><Input id={`material-stock-type-position-${defaultName || "new"}`} type="number" min={0} step={1} value={position} onChange={(event) => setPosition(event.target.value)} required /></div><DialogFooter><Button type="submit" disabled={saving}>Salvar</Button></DialogFooter></form></DialogContent></Dialog>;
}