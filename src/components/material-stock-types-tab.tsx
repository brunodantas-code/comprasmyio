...
export function MaterialStockTypesTab() {
  ...
  return <Collapsible open={expanded} onOpenChange={setExpanded} asChild><Card>
    <CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle>Tipos de Estoque</CardTitle>{expanded && <CardDescription>Os tipos ativos aparecem ao cadastrar um item novo em Materiais.</CardDescription>}</div><div className="flex items-center gap-1"><CollapsibleContent><StockTypeDialog title="Novo tipo de estoque" defaultPosition={Math.max(0, ...(types ?? []).map((item) => item.position)) + 1} saving={create.isPending} onSave={(name, position, destinationType) => create.mutateAsync({ name, position, destinationType })} trigger={<Button size="icon" aria-label="Criar tipo de estoque" title="Criar tipo"><Plus className="h-4 w-4" /></Button>} open={createOpen} onOpenChange={setCreateOpen} /></CollapsibleContent><CollapsibleTrigger asChild><Button size="icon" variant="ghost" aria-label={expanded ? "Recolher Tipos de Estoque" : "Expandir Tipos de Estoque"} title={expanded ? "Recolher" : "Expandir"}>{expanded ? <Minus className="h-4 w-4" /> : <Plus className="h-4 w-4" />}</Button></CollapsibleTrigger></div></CardHeader>
