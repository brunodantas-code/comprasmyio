...
function DeleteProfileDialog({ definition, definitions, deleting, onDelete }: { definition: AccessProfileDefinition; definitions: AccessProfileDefinition[]; deleting: boolean; onDelete: () => Promise<unknown> }) {
  ...
  const relocate = async () => { if (!destination) return toast.error("Selecione o novo perfil."); setMoving(true); const target = definitions.find((item) => item.code === destination); const { error } = await supabase.from("user_access_profiles").update({ profile_definition_id: destination, profile: target?.base_profile ?? "restrito" }).eq("profile_definition_id", definition.code); setMoving(false); if (error) return toast.error(error.message); setCount(0); toast.success("Usuários realocados"); };
